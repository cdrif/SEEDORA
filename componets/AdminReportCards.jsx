  ...
  useEffect(() => {
    async function initAdminPortal() {
      try {
        ...
        // Fetch academic grades (pre-filled from AcademicGradingPortal tables)
  ...
  return (
    <div className="min-h-screen bg-[#0b0f19] text-white print:bg-white print:text-black">
      ...
      <main className="max-w-6xl mx-auto px-6 py-8">
        ...
        {/* REPORT CARDS CONTAINER (Iterates over filtered students for batch or single print) */}
        {loading ? (
          ...
          <div className="space-y-12" ref={printRef}>
            {filteredStudents.map((student) => {
              const studentGrades = academicRecords.filter(r => r.student_id === student.id);
              const att = attendanceRecords[student.id] || { total_days: 180, attended: 172, absent_exp: 5, absent_unexp: 3 };
              
              // Calculate overall mock or derived GPA grade letter
              const overallGradeLetter = studentGrades.length > 0 ? studentGrades[0].overall_grade || 'B' : 'B+';
              ...
              return (
                <div key={student.id} className="bg-white text-black border border-gray-300 rounded-xl p-8 shadow-md print:shadow-none print:border-none print:p-0 print:mb-16 page-break-after">
                    ...
                    <table className="w-full text-left border-collapse border border-gray-300 text-sm">
                      <thead>
                        <tr className="bg-blue-900 text-white font-mono text-xs">
                          ...
                          <th className="py-2.5 px-4 border border-blue-900">Teacher</th>
                          <th className="py-2.5 px-4 border border-blue-900">Score</th>
                          <th className="py-2.5 px-4 border border-blue-900">Assessment Notes</th>
                        </tr>
                      </thead>
                      <tbody>
                        {studentGrades.length === 0 ? (
                          <tr>
                            <td colSpan="4" className="py-4 px-4 text-center text-gray-500 border border-gray-300 italic">
                              No academic records populated from grading portal yet. Showing placeholder standards.
                            </td>
                          </tr>
                        ) : (
                          studentGrades.map((g, idx) => (
                            <tr key={idx} className="border border-gray-300 odd:bg-gray-50">
                              <td className="py-2.5 px-4 border border-gray-300 font-bold text-gray-900">{g.subject}</td>
                              <td className="py-2.5 px-4 border border-gray-300 text-gray-700">{g.teacher_name || 'Staff Instructor'}</td>
                              <td className="py-2.5 px-4 border border-gray-300 font-mono font-semibold">{g.score || '75%'} ({g.letter_grade || 'B'})</td>
                              <td className="py-2.5 px-4 border border-gray-300 text-gray-600 text-xs">{g.assessment_notes || 'Consistent progress demonstrated across module tasks.'}</td>
                            </tr>
                          ))
                        )}
                      </tbody>
                  ...
                  <div className="mb-6 border border-gray-300 rounded-xl p-4 bg-gray-50/50 space-y-2 text-xs">
                    <h3 className="font-bold uppercase tracking-wider text-gray-900 mb-2">Performance Summary & Recommendations</h3>
                    <p><strong className="text-gray-900">Excelling in:</strong> {student.excelling_in || 'Active class participation, collaborative group assignments, and practical components.'}</p>
                    <p><strong className="text-gray-900">Areas to Improve:</strong> {student.needs_improvement || 'Pacing during timed assessments and assignment submission punctuality.'}</p>
                    <p><strong className="text-gray-900">Suggestions:</strong> {student.suggestions || 'Establish a structured revision timetable at home and minimize classroom distractions.'}</p>
                  </div>
                  ...
                  <div className="mb-8 border border-gray-300 rounded-xl p-3 bg-gray-100 text-center">
                    ...
                    <div className="grid grid-cols-5 gap-2 text-xs font-mono">
                      <div className="bg-white border rounded p-1"><strong>A:</strong> Very High</div>
                      <div className="bg-white border rounded p-1"><strong>B:</strong> High</div>
                      <div className="bg-white border rounded p-1"><strong>C:</strong> Sound</div>
                      <div className="bg-white border rounded p-1"><strong>D:</strong> Developing</div>
                  ...
                  <div className="grid grid-cols-2 gap-12 pt-4 border-t border-dashed border-gray-400 text-xs">
                    <div>
                      <div className="h-10 flex items-end font-mono text-gray-700 italic">J. Doe, Senior Instructor</div>
                      <div className="border-t border-gray-500 pt-1 text-gray-600">Teacher Signature & Date</div>
                    </div>
                    <div>
                      <div className="h-10 flex items-end font-mono text-gray-800 font-semibold">{principalSig || 'Dr. A. Vance, Principal'}</div>
